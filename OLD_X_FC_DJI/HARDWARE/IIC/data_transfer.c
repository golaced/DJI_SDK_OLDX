	...
	switch(state_mine)
	 {
		case 0:		
		 ...
		 #else 
		 ANO_DT_Send_Status(m100.Rol,m100.Pit,m100.Yaw,(0.1f *ultra_dis_lpf),fly_mode,fly_ready);	
		 #endif	 
		 }
		 else{cnt1=1;
			 #if USE_HT_GROUND 
    ...
    HT_DT_Send_RC(CH[2]+1500,CH[3]+1500,CH[0]+1500,CH[1]+1500,CH[4]+1500,CH[5]+1500,m100_data_refresh,m100.STATUS,m100.GPS_STATUS,state_v);
	 ...
	 case 1:
		 ...
		 if(sel[1]==0){sel[1]=1;
			...
			#else			 
		 ANO_DT_Send_Speed(m100.spd[0]*1000,m100.spd[1]*1000,m100.spd[2]*1000);
		 #endif
			 }
		 else{sel[1]=0;
			#if !USE_HT_GROUND 
...
{ u8 mode;
	...
	{
		//eso_pos_spd[Yr].b0=eso_pos_spd[X].b0  = ( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );
