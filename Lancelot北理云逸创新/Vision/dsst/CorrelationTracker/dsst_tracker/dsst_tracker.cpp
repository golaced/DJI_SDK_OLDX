...
dlib::dsst_tracker::dsst_tracker()
{
...
unsigned long dlib::dsst_tracker::get_filter_size() const
{
	return 64;
}

unsigned long dlib::dsst_tracker::get_num_scale_levels() const
{
	return 32;
}

unsigned long dlib::dsst_tracker::get_scale_window_size() const
{
	return 23;
}

double dlib::dsst_tracker::get_regularizer_space() const
{
	return 0.001;
}

double dlib::dsst_tracker::get_nu_space() const
{
	return 0.025;
}

double dlib::dsst_tracker::get_regularizer_scale() const
{
	return 0.001;
}

double dlib::dsst_tracker::get_nu_scale() const
{
	return 0.025;
}

dlib::drectangle dlib::dsst_tracker::get_position() const
{

	return position;
}

double dlib::dsst_tracker::get_scale_pyramid_alpha() const
{
	return 1.020;
}

double dlib::dsst_tracker::update(const image_type& img, const drectangle& guess)
{
...
void dlib::dsst_tracker::make_target_location_image(const dlib::vector<double, 2>& p, matrix<std::complex<double> >& g) const
{
...
void dlib::dsst_tracker::make_scale_target_location_image(const double scale, matrix<std::complex<double>, 0, 1>& g) const
{
...
dlib::matrix<double> dlib::dsst_tracker::make_cosine_mask() const
{
...
double dlib::dsst_tracker::update(const image_type& img)
{
	return update(img, get_position());
}


void dlib::dsst_tracker::start_track(const image_type& img, const drectangle& p)
{
...
void dlib::dsst_tracker::make_scale_space(const image_type& img, std::vector<matrix<std::complex<double>, 0, 1> >& Fs) const
{
	typedef image_traits<image_type>::pixel_type pixel_type;
	...
	for (unsigned long i = 0; i < get_num_scale_levels(); ++i)
	{
		...
		dlib::transform_image(img, chip, interpolate_bilinear(), find_affine_transform(from_points, to_points));
...
dlib::point_transform_affine dlib::dsst_tracker::make_chip(const image_type& img, drectangle p, std::vector<matrix<std::complex<double> > >& chip) const
{
	typedef image_traits<image_type>::pixel_type pixel_type;
