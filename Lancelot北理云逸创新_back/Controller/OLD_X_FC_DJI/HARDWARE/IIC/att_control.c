   ...
   for (axis = 0; axis < 2; axis++) {
       ...
       //-----------rad
        p = rate_error[axis]*p_in_use;//pid.nav.in.p;
	     ...
	     if((ABS(CH_filter[ROLr])<100)&&(ABS(CH_filter[PITr])<100)&&ABS(Pitch)<15&&ABS(Roll)<15)
    ...
    // add in derivative component
       d_flow_watch[axis]= d = derivative[axis]*pid.nav.in.d;//d
        ...
        // get rid of noise
        //if (ABS(actual_speed[axis]) < pid.nav.in.dead)
         //   d = 0;
...
//---------------------------NAV_angle------------------------------------	
if(!mode.dj_lock)	{
	if(mode.flow_hold_position&&((ABS(except_A_SB[PITr])<2)&&(ABS(except_A_SB[ROLr])<2))
	...
	if(mode.flow_hold_position&&
			((ABS(dj_sb)<2)&&(ABS(except_A_SB[ROLr])<2))
			&&NAV_BOARD_CONNECT==1)//add by gol 2015.10.22
			{
...
//	}
	//-----------------ATT_PROTECTOR----------------------------------------
	if(ABS(Pitch)>MAX_CTRL_ANGLE*1.5||ABS(Roll)>MAX_CTRL_ANGLE*1.5||ultra_distance<400)
	dj_angle_set=0;
...
//--------------------------超声波 壁障----------------------------------------------
//	if(mode.sonar_avoid&&((ABS(except_A_SB_lft[PIT])<2)&&(ABS(except_A_SB_lft[ROL])<2))&&	(ultra_dis_lpf>800))//&&need_avoid)
//	{
...
//	);
 ...
 // except_A.z  = To_180_degrees(Yaw_set_dji);//---------------------
  /* 得到角度误差 */
	...
	// except_A.z  = To_180_degrees(Yaw_set_dji);//---------------------
	if(mode.en_dji_yaw&&ALT_POS_SONAR2>0.3&&ABS(Rc_Pwm_Inr_mine[RC_YAW]-OFF_RC_YAW)<80)
	#if  USE_M100
...
//	ctrl_2.PID[PIDROLL].kd=0;
	//------------------YAW PROTECTOR--------------------------------
//	if(ABS(To_180_degrees(except_A.z - Yaw))>80){
//		except_A.z  = To_180_degrees(Yaw);
//	  ctrl_2.err.z=0;
//	}
...
#define SHOOT_DIRECT 1//直接射击   连续
#define CHECK_NUM_DIS_WITH_LASER 1
#define USE_OVER_TIME_FOR_STATE 0//使用MAP状态超时
#define CHECK_SCREEN_WHILE_FLYING 1//在巡航状态也检测屏幕
#define USE_MISS_SCAN 1 //使用丢失左右侧飞
#define SHOOTING_TIME 8.88//射击时间
#define MAX_TARGET 5//最多设计数字
...
//state_change 
	...
	{//-------------------------------------------------起飞
		case SG_LOW_CHECK://low thr check
			shoot_finish_cnt=cnt_state_over_time_map=miss_state=cnt_state_over_time=fake_target_flag=fly_cover_cnt=cnt_shoot=over_time=cnt_back_home=cnt_map_home=0;tar_need_to_check_odroid[2]=66;
		  tar_cnt=0;
		  for(i=0;i<19;i++)
		    tar_buf[i]=66;
...
//超时处理	
	...
	#endif
	#if USE_M100
	if(cnt_back_home>7.66*60/T)//超时降落	
	{over_time=2;}
...
//circle ——check	
		if(circle.check&&circle.connect&&ABS(circle_use[0])<DEAD_CIRCLE_CHECK&&ABS(circle_use[1])<DEAD_CIRCLE_CHECK)
